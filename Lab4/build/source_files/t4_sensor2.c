...
static char str_ONE_SHOT_POT_PRE[] = "Potentiometer reading: ";

static esos_sensor_process_t processing_mode = ESOS_SENSOR_ONE_SHOT;
...
ESOS_USER_TASK(SerialOutput) {
    ...
    while (1) {
        if(MENU_STATE == INSTRUCT) {
			...
			//one-shot
			if(mode == '1'){processing_mode = ESOS_SENSOR_ONE_SHOT;}
			//avg
			else if(mode == '2' && sample_size == '1'){processing_mode = ESOS_SENSOR_AVG2;}
